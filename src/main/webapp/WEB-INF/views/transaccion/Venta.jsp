...
<html lang="es">
  ...
  <body>
    <header id="navbar"><%@ include file="../componentes/navbar_bodega.jsp" %></header>
          ...
          <table class="tbl">
            ...
            <tbody>
              ...
              <c:if test="${empty ventas}">
                <tr>
                  <td colspan="4" class="muted">No hay ventas registradas</td>
                </tr>
              </c:if>
            </tbody>
          </table>
    ...
    <section id="m-agregar" class="modal">
      <div class="box">
        ...
        <p class="muted">
          Busca por código o nombre y selecciona el producto desde la lista (datos reales desde BD).
        </p>
    ...
    <script>
      ...
      function crearFilaDetalle(d, i) {
        ...
        inner +=
          '<td class="c"><button type="button" class="icon" onclick="quitarDelCarrito(' +
          i +
          ')">🗑</button></td>'
        tr.innerHTML = inner
        return tr
      ...
      function renderCarrito() {
        ...
        var descuentoVisible = parseFloat(
          document.getElementById('inputDescuentoVisible').value || 0
        )
      ...
      function buildHiddenInputs() {
        ...
        for (var i = 0; i < detalles.length; i++) {
          ...
          container.insertAdjacentHTML(
            'beforeend',
            '<input type="hidden" name="productoIds[]" value="' + (d.idProducto || '') + '" />'
          )
          container.insertAdjacentHTML(
            'beforeend',
          ...
          ;(function (p) {
            ...
            div.addEventListener('click', function () {
              ...
              document.getElementById('modal_stock').textContent =
                'Stock: ' + (p.stockActual != null ? p.stockActual : 'N/A')
              out.innerHTML = ''
      ...
      document.addEventListener('click', function (e) {
        ...
        if (e.target && e.target.id === 'btnAbrirAgregar') {
          ...
          setTimeout(function () {
            document.getElementById('modal_buscar').focus()
          }, 100)
        }
      })
      ...
      document.getElementById('btnAgregarModal').addEventListener('click', function () {
        var idProd = document.getElementById('modal_producto_id').value
          ? parseInt(document.getElementById('modal_producto_id').value, 10)
          : null
        ...
        if (!idProd) {
          alert('Selecciona un producto desde la lista (busca y selecciona).')
          return
        }
        if (cantidad <= 0) {
